...
const project = {
  ...
  links: { repo: "https://github.com/x/simple-tools" },
...
describe("StageWindow.astro", () => {
  ...
  it("seeds the media caption footer with the first capture caption and total", async () => {
    const html = await render({ project });
    expect(html).toContain("stage-caption");
    expect(html).toContain("data-view-caption");
    expect(html).toContain("Image Compressor");
    expect(html).toMatch(/data-view-pos[^>]*>1 \/ 2</);
  });

  it("renders project stack and links in the chrome rail", async () => {
    const html = await render({ project });
    expect(html).toContain("stack-icons--chrome");
    expect(html).toContain("projlinks--chrome");
    expect(html).toContain('href="/projects/simple-tools/"');
